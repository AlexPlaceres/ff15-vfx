...
public class FlatBufferBuilder
{
    ...
    public FlatBufferBuilder(int initialSize)
    {
        ...
        _space = initialSize;
        _bb = new FlatBuffer(MemoryOwner<byte>.Allocate(initialSize));
    }
    ...
    void GrowBuffer()
    {
        ...
        var newBuf = MemoryOwner<byte>.Allocate(newBufSize);

        // WARN: I have no clue what I'm doing
        oldBuf.CopyTo(newBuf.Span.Slice(newBufSize - oldBufSize, oldBufSize));

        // Buffer.BlockCopy(oldBuf, 0, newBuf, newBufSize - oldBufSize, oldBufSize);
        _bb = new FlatBuffer(newBuf, newBufSize);
    }
    ...
    {
        _bb.PutU8(_space -= sizeof(byte), (byte)(x ? 1 : 0));
    }

    public void PutSbyte(sbyte x)
    {
        _bb.PutS8(_space -= sizeof(sbyte), x);
    }

    public void PutByte(byte x)
    {
        _bb.PutU8(_space -= sizeof(byte), x);
    }

    public void PutShort(short x)
    {
        _bb.PutS16(_space -= sizeof(short), x);
    }

    public void PutUshort(ushort x)
    {
        _bb.PutU16(_space -= sizeof(ushort), x);
    }

    public void PutInt(int x)
    {
        _bb.PutS32(_space -= sizeof(int), x);
    }

    public void PutUint(uint x)
    {
        _bb.PutU32(_space -= sizeof(uint), x);
    }

    public void PutLong(long x)
    {
        _bb.PutS64(_space -= sizeof(long), x);
    }

    public void PutUlong(ulong x)
    {
        _bb.PutU64(_space -= sizeof(ulong), x);
    }
    ...
    public StringOffset CreateString(string s)
    {
        ...
        // WARN: Potentially bad
        Encoding.UTF8.GetBytes(s.AsSpan(), _bb.Data.Slice(_space -= utf8StringLen));
    ...
    public int EndObject()
    {
        ...
        AddShort((short)((_vtableSize + standardFields) * sizeof(short)));
        ...
        for (int i = 0; i < _numVtables; i++)
        {
            ...
            int vt2 = _space;
            short len = _bb.ReadS16(vt1);
            if (len == _bb.ReadS16(vt2))
            {
                for (int j = sizeof(short); j < len; j += sizeof(short))
                {
                    if (_bb.ReadS16(vt1 + j) != _bb.ReadS16(vt2 + j))
                    {
                        goto endLoop;
                    }
                }
        ...
        if (existingVtable != 0)
        {
            ...
            _bb.PutS32(_space, existingVtable - vtableloc);
        }
        else
        {
            // No match:
            ...
            _bb.PutS32(_bb.Length - vtableloc, Offset - vtableloc);
        }
    ...
    public void Required(int table, int field)
    {
        ...
        int vtable_start = table_start - _bb.ReadS32(table_start);
        bool ok = _bb.ReadS16(vtable_start + field) != 0;
    ...
    public ReadOnlySpan<byte> SizedSpan()
    {
        return _bb.Data.Slice(_bb.Position, _bb.Length - _bb.Position);
    }
