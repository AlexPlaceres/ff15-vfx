    ...
    public class FlatBufferBuilder
    {
            ...
            where T : struct
        {
            ...
            if (sizeInBytes < 0)
            {
                throw new ArgumentOutOfRangeException(
                    "sizeInBytes",
                    "sizeInBytes cannot be negative"
                );
            }
            ...
            if ((sizeInBytes % size) != 0)
            {
                throw new ArgumentException(
                    "The given size in bytes "
                        + sizeInBytes
                        + " doesn't match the element size of T ( "
                        + size
                        + ")",
                    "sizeInBytes"
                );
        ...
        public int EndTable()
        {
            ...
            int i = _vtableSize - 1;
            // NOTE: For compatibility with FFXV,
            // vtables shouldn't be trimmed

            // Trim trailing zeroes.
            // for (; i >= 0 && _vtable[i] == 0; i--) {}
            // int trimmedSize = i + 1;
            int trimmedSize = _vtableSize;
            for (; i >= 0; i--)
            {
            ...
            AddShort((short)((trimmedSize + standardFields) * sizeof(short)));
